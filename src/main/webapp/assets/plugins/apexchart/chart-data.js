...
$(document).ready(function () {
    ...
    if ($('#sales_charts').length > 0) {
        var salesChartEl = document.querySelector("#sales_charts");
        var parseChartData = function (value) {
            try {
        ...
        var normalizeMonthlyData = function (data) {
            var result = [];
            for (var i = 0; i < 12; i++) {
                result.push(Number(data[i]) || 0)
            }
            return result
        };
        var toMillions = function (value) {
            return Math.round((value / 1000000) * 100) / 100
        };
        var formatFullMoney = function (value) {
            return Math.round(Math.abs(value) * 1000000).toLocaleString('vi-VN')
        };
        var salesData = normalizeMonthlyData(parseChartData(salesChartEl.dataset.sales)).map(toMillions);
        var purchaseData = normalizeMonthlyData(parseChartData(salesChartEl.dataset.purchase)).map(function (value) {
            return -toMillions(value)
        });
        var options = {
            series: [{name: 'Sales', data: salesData,}, {name: 'Purchase', data: purchaseData}],
