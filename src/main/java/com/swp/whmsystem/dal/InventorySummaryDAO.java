...
public class InventorySummaryDAO {
    ...
    public List<InventorySummary> forReport(String openDate, String closeDate, String keyword, int page, int pageSize, String sortColumn, String sortOrder) {
        List<InventorySummary> list = new ArrayList<>();
        Timestamp tOpenStart = toStartTimestamp(openDate);
        Timestamp tCloseMax = toEndTimestamp(closeDate);
        String kw = toKeywordPattern(keyword);
        ...
        if (sortColumn != null && !sortColumn.trim().isEmpty()) {
            ...
            switch (sortColumn) {
                case "openingStock":
                    sql.append("ORDER BY opening_stock ").append(order).append(", p.productid DESC ");
                    break;
                case "importStock":
                    sql.append("ORDER BY import_stock ").append(order).append(", p.productid DESC ");
                    break;
                case "exportStock":
                    sql.append("ORDER BY export_stock ").append(order).append(", p.productid DESC ");
                    break;
                case "closingStock":
                    sql.append("ORDER BY closing_stock ").append(order).append(", p.productid DESC ");
                    break;
                default:
    ...
    private List<InventorySummary> getTop5ByType(String openDate, String closeDate, String type) {
        List<InventorySummary> list = new ArrayList<>();
        Timestamp tStart = toStartTimestamp(openDate);
        Timestamp tEnd = toEndTimestamp(closeDate);
